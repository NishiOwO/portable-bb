...
static int start_sound(void)
{
	...
	if(system(str)) {
	  sprintf(str, "./bb_snd_server %i %i %i bb.s3m bb2.s3m bb3.s3m", freq, stereo, _16bit);
	  if(system(str)) {
	    sprintf(str, "./bin/Debug/bb_snd_server %i %i %i bb.s3m bb2.s3m bb3.s3m", freq, stereo, _16bit);
	    if(system(str)) {
	      sprintf(str, "./bin/Release/bb_snd_server %i %i %i bb.s3m bb2.s3m bb3.s3m", freq, stereo, _16bit);
	      system(str);
	    }
	  }
	}
